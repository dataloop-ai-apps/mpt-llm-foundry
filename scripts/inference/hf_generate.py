...
def main(args: Namespace) -> list:
    ...
    maybe_synchronize()
    gen_start = time.time()
    print(f"Length of encoded_inp: {len(encoded_inp)}")
    encoded_gen = _generate(encoded_inp)
    print(f"Length of encoded_gen: {len(encoded_gen)}")
    maybe_synchronize()
    gen_end = time.time()

    decode_start = time.time()
    decoded_gen = tokenizer.batch_decode(encoded_gen, skip_special_tokens=True)
    print(f"Length of decoded_gen: {len(decoded_gen)}")
    maybe_synchronize()

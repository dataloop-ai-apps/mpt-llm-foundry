...
def get_hf_config_from_composer_state_dict(
    ...
    print(f'WHAT ARE WE REALLY GETTING IN HF_CONFIG_DICT: {hf_config_dict}')
    ...
    if 'attn_config' not in hf_config_dict:
        ...
        hf_config_dict = clean_tag(hf_config_dict, 'attn_pdrop')
        attn_config['attn_impl'] = hf_config_dict.get('attn_impl', 'torch')
        hf_config_dict = clean_tag(hf_config_dict, 'attn_impl')
        attn_config['qk_ln'] = hf_config_dict.get('attn_qk_ln')
        hf_config_dict = clean_tag(hf_config_dict, 'attn_qk_ln')
        attn_config['clip_qkv'] = hf_config_dict.get('attn_clip_qkv')
        hf_config_dict = clean_tag(hf_config_dict, 'attn_clip_qkv')
        attn_config['softmax_scale'] = hf_config_dict.get('softmax_scale')
        hf_config_dict = clean_tag(hf_config_dict, 'softmax_scale')
        attn_config['prefix_lm'] = hf_config_dict.get('prefix_lm')
        hf_config_dict = clean_tag(hf_config_dict, 'prefix_lm')
        attn_config['attn_uses_sequence_id'] = hf_config_dict.get('attn_uses_sequence_id')
        hf_config_dict = clean_tag(hf_config_dict, 'attn_uses_sequence_id')
        attn_config['alibi'] = hf_config_dict.get('alibi')
        hf_config_dict = clean_tag(hf_config_dict, 'alibi')
        attn_config['alibi_bias_max'] = hf_config_dict.get('alibi_bias_max')
        hf_config_dict = clean_tag(hf_config_dict, 'alibi_bias_max')
    ...
    if 'init_config' not in hf_config_dict:
        ...
        init_config['name'] = hf_config_dict.get('param_init_fn', "default_")
        hf_config_dict = clean_tag(hf_config_dict, 'param_init_fn')
        init_config['fan_mode'] = hf_config_dict.get('fan_mode')
        hf_config_dict = clean_tag(hf_config_dict, 'fan_mode')
        init_config['init_nonlinearity'] = hf_config_dict.get('init_nonlinearity')
        hf_config_dict = clean_tag(hf_config_dict, 'init_nonlinearity')
        init_config['init_gain'] = hf_config_dict.get('init_gain')
        hf_config_dict = clean_tag(hf_config_dict, 'init_gain')
        init_config['init_std'] = hf_config_dict.get('init_std')
        hf_config_dict = clean_tag(hf_config_dict, 'init_std')
        init_config['init_div_is_residual'] = hf_config_dict.get('init_div_is_residual')
        hf_config_dict = clean_tag(hf_config_dict, 'init_div_is_residual')
        init_config['emb_init_std'] = hf_config_dict.get('emb_init_std')
        hf_config_dict = clean_tag(hf_config_dict, 'emb_init_std')
        init_config['emb_init_uniform_lim'] = hf_config_dict.get('emb_init_uniform_lim')
        hf_config_dict = clean_tag(hf_config_dict, 'emb_init_uniform_lim')
    ...
    if 'mlp_ratio' in hf_config_dict:
        ...
        hf_config_dict = clean_tag(hf_config_dict, 'mlp_ratio')

    if 'low_precision_layernorm' in hf_config_dict:
        if hf_config_dict.get('low_precision_layernorm'):
            hf_config_dict['norm_type'] = 'low_precision_layernorm'
            hf_config_dict = clean_tag(hf_config_dict, 'low_precision_layernorm')
        else:
...
def write_huggingface_pretrained_from_composer_checkpoint(
    ...
    print(f"OBJECT STORE: {object_store}")
...
def main(args: Namespace) -> None:
    ...
    print(f"Output path: {args.hf_output_path}")
    print(f"Composer path: {args.composer_path}")

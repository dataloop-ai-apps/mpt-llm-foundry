...
def build_dataloader(cfg, tokenizer, device_batch_size):
    if cfg.name == 'text':
        ...
        logger.debug(f" $$$$$$$$$$$$$$$$ HERE IS IS MESDAMES ET MESSIEURS DATASET NAME {cfg.dataset.hf_name}")
...
def main(cfg, current_model=None, current_tokenizer=None):
    # Check for incompatibilities between the model and data loaders
    os.listdir(cfg.data_local)
    validate_config(cfg)
    ...
    # build tokenizer
    tokenizer = build_tokenizer(cfg.tokenizer) if not current_tokenizer else current_tokenizer
    ...
    with init_context:
        t = tokenizer if cfg.model.name != 'hf_t5' else cfg.tokenizer
        model = build_composer_model(cfg.model, t) if not current_model else current_model
    cfg.n_params = sum(p.numel() for p in model.parameters())
    print(f'{cfg.n_params=:.2e}')

    # Dataloaders
    print('Building train loader...')
    print(f"FILES IN TRAIN LOADER {cfg.train_loader.get('dataset', {}).get('local')}: {os.listdir(cfg.train_loader.get('dataset', {}).get('local'))}")
    if 'train' in os.listdir(cfg.train_loader.get('dataset', {}).get('local')):
        print(f"CONTENTES OF TRAIN: {os.listdir(os.path.join(cfg.train_loader.get('dataset', {}).get('local'), 'train'))}")
    else:
        print("WHY WAS TRAIN NOT IN THAT DIRECTORY????")
    print("***********")
    print("Before building train dataloader:")
    print(f"cfg.train_loader: {cfg.train_loader}")
    print(f"tokenizer: {tokenizer}")
    print(f"cfg.device_train_batch_size: {cfg.device_train_batch_size}")
    train_loader = build_dataloader(
        cfg.train_loader,
        tokenizer,
        cfg.device_train_batch_size,
    )
    print(f"Amount of items in train dataloader: {len(train_loader)}")
    print('Building eval loader...')
    evaluators = []
    if 'eval_loader' in cfg and False:
